...
require_once "functions/GetTrail.php";
          ...
          <div class="card">
            ...
            <div class="col-12">
              <div class=" info-card revenue-card ">
                ...
                <h5 class="card-title text-center"> طلاب ليس لديهم عمليات دفع ( تجريبي )</h5>
                  <button  data-bs-target="#GetRenewal" aria-expanded="false" aria-controls="GetRenewal" class="bg-secondary text-white card-icon w-50 rounded-pill border  border-dark d-flex align-items-center justify-content-center " >
                    <h6 class="text-center text-white rtl"><?php echo  count(GetTrail($Who))."  "."طالب" ; ?></h6>
                  </button>
                  <h5 class="card-title text-center"> المطلوب: متابعتهم وتحصيل الدفع </h5>
                </div>
              </div>
            </div>
            ...
            <div class="card-body overflow-auto text-center"  id="GetRenewal" data-bs-parent="#myGroup">
              <table  class="table table-border table-hover text-center DataTable">
                ...
                <tbody>
                  <?php  if (count(GetTrail($Who)) == 0) { ?>  <td  class="fw-bold fs-5" colspan="3"> لا توجد طلبات مسجلة </td>  <?php } ?>
                  <?php 
                  if (count(GetTrail($Who)) > 0) {

                    foreach ( GetTrail($Who) as $value) {
