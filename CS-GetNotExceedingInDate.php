          ...
          <div class="card">
            <div class="col-12">
              <div class=" info-card revenue-card ">
                ...
                <h5 class="card-title text-center"> طلاب لهم حلقات وحان موعد تجديدهم </h5>
                  <button type="button" data-bs-toggle="collapse" data-bs-target="#GetNotExceedingInDate" aria-expanded="false" aria-controls="GetNotExceedingInDate" class="bg-secondary text-white card-icon w-50 rounded-pill border  border-dark d-flex align-items-center justify-content-center " >
                    <h6 class="text-center text-white rtl"><?php echo  count(GetNotExceedingInDate($Who))."  "."طالب" ; ?></h6>
                  </button>
            ...
            <div class="card-body overflow-auto text-center"  id="GetNotExceedingInDate" data-bs-parent="#myGroup">
              <table  class="table table-border table-hover text-center DataTable">
                ...
                <tbody>
                  <?php  if (count(GetNotExceedingInDate($Who)) == 0) { ?>  <td  class="fw-bold fs-5" colspan="3"> لا توجد طلبات مسجلة </td>  <?php } ?>
                  <?php 
                  if (count(GetNotExceedingInDate($Who)) > 0) {

                    foreach ( GetNotExceedingInDate($Who) as $value) {
