    ...
    <section class="section dashboard">
      ...
      <div class="col-lg-12">
          <div class="card">
            ...
            <div class="col-12">
              <div class=" info-card revenue-card ">
                <div class="card-body d-flex flex-column align-items-center justify-content-center">
                  <h5 class="card-title text-center"> طلاب تجاوزا عدد حصص الباقة الخاصة بهم وقد حان موعد تجديدهم </h5>
                  <button type="button" data-bs-toggle="collapse" data-bs-target="#GetExceedingInDate" aria-expanded="false" aria-controls="GetExceedingInDate" class="bg-secondary text-white card-icon w-50 rounded-pill border  border-dark d-flex align-items-center justify-content-center " >
                    <h6 class="text-center text-white rtl"><?php echo  count(GetExceedingInDate($Who))."  "."طالب" ; ?></h6>
                  </button>
            ...
            <div class="card-body  overflow-auto text-center"  id="GetExceedingInDate" data-bs-parent="#myGroup">
              <table  class="table table-border table-hover text-center DataTable">
                <thead >
                  <tr>
                    ...
                    <th   class="fw-bold fs-5 text-center text-nowrap" > الحصص آخر اسبوعين </th>
                  </tr>
                </thead>
                <tbody>
                  <?php  if (count(GetExceedingInDate($Who)) == 0) { ?>  <td  class="fw-bold fs-5" colspan="3"> لا توجد طلبات مسجلة </td>  <?php } ?>
                  <?php 
                  if (count(GetExceedingInDate($Who)) > 0) {

                    foreach ( GetExceedingInDate($Who) as $value) { 
                    ...
                    <tr onclick="window.location='<?php echo 'search?Code='.$StudentCodeExplode?>';" role="button">
                      <td class="fw-bold fs-5 text-start text-nowrap"> <?php echo $value['Code'] ?> </td>
                      ...
                      <td class="fw-bold fs-5 text-center text-nowrap"> <?php echo $value['LastClasses'] ?> </td>
                    </tr>
                    <?php }?>
                  <?php } ?>
                </tbody>
