...
$evaluation =array();
  $users_stmt = $con->prepare("SELECT * FROM users WHERE Code = ? OR Code = ? ORDER BY `Code` ASC  ");
  $users_stmt->execute(array('a.mangod','Amr'));
...
$plus_user_stmt->execute(array($user['Code'],"+",$this_month,$Next_month,5));
...
$minus_user_stmt->execute(array($user['Code'],"-",$this_month,$Next_month,5));
...
$evaluation_user_stmt->execute(array($user['Code'],$this_month,$Next_month,5));
