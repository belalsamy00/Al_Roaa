...
$Active_stmt = $con->prepare("SELECT * FROM ArchivedStudents $stmt");
        ...
        <div class="col-12 m-auto ">
          ...
          <div class="card">
            <div class="card-body rtl overflow-auto text-center">
              <h5 class="card-title text-center"> جميع الأكواد المؤرشفة </h5>

              <h5 class="rtl text-center"> الأكواد المؤرشفة هى أكواد متوقفة ولم تأخذ حصص لمدة أربعة اشهر </h5>
              <p class="rtl text-center"> اذا اراد الطالب الاستئناف قدم طلب لاستخراج الكود من الأررشيف </p>
              <table id="DataTable"   class="table table-danger table-border table-hover text-center  DataTable  " >
                <thead >
                  <tr >
                  ...
                  <th class= text-center>الحصص المسجلة</th>
                  <th class= text-center>الحصص المتبقية</th>
                  <th class= "text-center text-nowrap">تاريخ اخر حصة</th>
                  <?php } ?>
                  </tr>
                </thead>
                <tbody>
                  ...
                  $Renewal_date = $data['Last_class'];
                  $Remained = $data['Remained'];
                  } ?>
                  <tr onclick="window.location='<?php echo 'ArchivedStudentsSearch?Code='.$StudentCodeExplode?>';" role="button">
                  ...
                  <th class="text-nowrap" style="<?php if ($Remained < 0) { echo ' color: green;';}elseif ($Remained > 0) { echo ' color: red;';}else {}?>" ><?php echo $Remained ; ?></th>
