            ...
            <div class="col-12">
              <div class=" info-card revenue-card ">
                <div class="card-body d-flex flex-column align-items-center justify-content-center">
                  <h5 class="card-title text-center"> طلاب غير مفعلين  </h5>
                  <button  data-bs-target="#GetRenewal" aria-expanded="false" aria-controls="GetRenewal" class="bg-secondary text-white card-icon w-50 rounded-pill border  border-dark d-flex align-items-center justify-content-center " >
                    <h6 class="text-center text-white rtl"><?php echo  count(GetCancel($Who))."  "."طالب" ; ?></h6>
                  </button>
                  <h5 class="card-title text-center"> المطلوب:  مراجعتهم لحذف بيانات المتغيبيين منذ زمن  </h5>
                </div>
              </div>
            </div>
              ...
              <table  class="table table-border table-hover text-center DataTable">
                <thead >
                  <tr>
                    ...
                    <th   class="fw-bold fs-5 text-center text-nowrap" > تاريخ آخر حصة </th>
                  </tr>
                </thead>
                <tbody>
                  <?php  if (count(GetCancel($Who)) == 0) { ?>  <td  class="fw-bold fs-5" colspan="3"> لا توجد طلبات مسجلة </td>  <?php } ?>
                  <?php 
                  if (count(GetCancel($Who)) > 0) {

                    foreach ( GetCancel($Who) as $value) {
