...
<main id="main" class="main">
  <div class="container">
    ...
    <section class="section">
      <div class="row">
        <?php foreach(glob('BackupFiles/*.*') as $filename){
                $n_ex= explode('-',$filename);
                $n_ex[0] = str_replace("BackupFiles/","",$n_ex[0]);
                $n_ex[6] = str_replace("DB","All Database Tables",$n_ex[6]);
            ?> <span class="d-flex  justify-content-between mb-2"> <h3 class="w-75"> <?php echo $n_ex[0]."-".$n_ex[1]."-".$n_ex[2]." / ".$n_ex[3].":".$n_ex[4].":".$n_ex[5]." / ".$n_ex[6]." ".$n_ex[8]; ?></h3>  
            <form class="row"  action="<?php echo $filename; ?>" method="get">
            <div class="col-6">
              <div class="input-group">
                <input type="text" class="form-control" name="pass" id="password" placeholder="password" required='true'>
              </div>
            </div>

            <div class="col-6">
              <button type="submit" class="btn btn-primary">Restore This Copy</button>
            </div>
          </form>
